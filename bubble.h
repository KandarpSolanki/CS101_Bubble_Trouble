...
public:
    ...
    void nextStep(double t)
    {
        ...
        double new_y = circle.getY() + vy*t + 0.5*20*t*t; // Displacement = ut + 1/2*a*t^2
        vy += 20*t; // V = U + gt
        ...
        if ((new_y > (450 - circle.getRadius()))) // bounce along Y direction at PLAY_Y_HEIGHT (y=450) (Blue line)
        {
            
            vy = -vy; //Change velocity direction when it hits bottom line
            new_y = circle.getY() + vy*t + 0.5*20*t*t; //new y-position path of the bubble
            
        }

        if ((new_y < (40 + circle.getRadius()))) // bounce along Y direction around top margin of window
        {
        ...
        circle.moveTo(new_x, new_y); //Move the bubble to the next position after STEP_TIME
    }
